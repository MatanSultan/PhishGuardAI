...
import type { AdminOverviewPayload } from '@/lib/admin/service'
import type {
...
const TeamProgressChart = dynamic(() => import('./team-progress-chart'), {
  ssr: false,
...
export default function AdminPageClient({
  initialData,
  initialError,
  initialRequiresAdmin,
}: AdminPageClientProps) {
  const { locale, dir } = useLocale()
  const companyCopy = getCompanyCopy(locale)
  const [data, setData] = useState<DashboardPayload | null>(initialData?.dashboard ?? null)
  const [simulationData, setSimulationData] = useState<SimulationPreviewPayload | null>(
    initialData ? { simulations: initialData.simulations } : null,
  )
  const [members, setMembers] = useState<OrganizationMemberRecord[]>(initialData?.members ?? [])
  const [invites, setInvites] = useState<DbRow<'team_invites'>[]>(initialData?.invites ?? [])
  const [membersError, setMembersError] = useState<string | null>(null)
  const [invitesError, setInvitesError] = useState<string | null>(null)
  const [isLoading, setIsLoading] = useState(!initialData && !initialError && !initialRequiresAdmin)
  const [error, setError] = useState<string | null>(initialError)
  const [requiresAdmin, setRequiresAdmin] = useState(initialRequiresAdmin)
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        if (!response.ok) {
          ...
          setError(payload?.error ?? companyCopy.admin.loadError)
          setIsLoading(false)
          return
        }

        setData((payload as AdminOverviewPayload).dashboard)
        setSimulationData({ simulations: (payload as AdminOverviewPayload).simulations ?? [] })
        setMembers((payload as AdminOverviewPayload).members ?? [])
        setInvites((payload as AdminOverviewPayload).invites ?? [])
        setIsLoading(false)
      } catch {
        if (!active) {
          return
    ...
    if (refreshKey === 0 && (initialData || initialError || initialRequiresAdmin)) {
      return () => {
        active = false
      }
    }
    ...
    <>
      ...
      <Card>
        ...
        <CardContent className="h-80">
          {hasTrendData ? (
            <TeamProgressChart data={data.teamProgressTrend} />
          ) : (
