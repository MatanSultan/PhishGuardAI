...
interface AdminReportsPageClientProps {
  initialData: ReportsPayload | null
...
export default function AdminReportsPageClient({
...
}: AdminReportsPageClientProps) {
  const { locale, dir } = useLocale()
  const companyCopy = getCompanyCopy(locale)
  const [data, setData] = useState<ReportsPayload | null>(initialData)
  ...
  useEffect(() => {
    ...
    if (
      ...
      !queryString &&
      (initialData || initialError || initialRequiresAdmin)
    ) {
  ...
  }, [companyCopy.reports.loadError, initialData, initialError, initialRequiresAdmin, queryString])
