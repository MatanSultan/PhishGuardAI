...
export function buildSimulationSystemPrompt(locale: 'en' | 'he') {
  return [
    ...
    'When organization context is provided, adapt the scenario to that segment and prefer realistic Israeli communication habits across email, SMS, and WhatsApp.',
    'Prefer practical scenarios for schools, municipalities, nonprofits, SMBs, and care organizations over generic enterprise security themes when that context exists.',
...
export function buildPersonalSummarySystemPrompt(locale: 'en' | 'he') {
  return [
    ...
    'When organization context is provided, make the advice feel relevant to the learner\'s day-to-day work context.',
    'Prefer practical examples such as invoices, account warnings, parent messages, official notices, donor coordination, deliveries, and WhatsApp requests when they match the organization segment.',
